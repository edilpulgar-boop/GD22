...
const sceneViewEl = document.getElementById('sceneView');
const sceneCanvasEl = document.getElementById('sceneCanvas');
const scenePlayBtn = document.getElementById('scenePlayBtn');
const sceneStopBtn = document.getElementById('sceneStopBtn');
const sceneExitBtn = document.getElementById('sceneExitBtn');
const sceneErrorEl = document.getElementById('sceneError');
const sceneTouchControlsEl = document.getElementById('sceneTouchControls');
const sceneTouchLeftBtn = document.getElementById('sceneTouchLeft');
const sceneTouchRightBtn = document.getElementById('sceneTouchRight');
const sceneTouchJumpBtn = document.getElementById('sceneTouchJump');
const docTooltipEl = document.getElementById('docTooltip');
...
// ---- guardar el código automáticamente (localStorage) ----
const CODE_STORAGE_KEY = 'gdconsola_saved_code';
let saveCodeTimer = null;

function saveCodeNow() {
  try { localStorage.setItem(CODE_STORAGE_KEY, codeEl.value); } catch (e) { /* almacenamiento no disponible, no pasa nada */ }
}
function scheduleSaveCode() {
  clearTimeout(saveCodeTimer);
  saveCodeTimer = setTimeout(saveCodeNow, 400);
}
function loadSavedCode() {
  try { return localStorage.getItem(CODE_STORAGE_KEY); } catch (e) { return null; }
}
...
// ---- documentación: "modo consulta" (🔍) ----
function getWordAtPosition(pos) {
  const text = codeEl.value;
...
function hideDocTooltip() {
  docTooltipEl.classList.remove('open');
  docTooltipEl.innerHTML = '';
}

function showDocTooltip(word, clientX, clientY) {
  const senses = DOCS[word];
  if (!senses) { hideDocTooltip(); return; }

  docTooltipEl.innerHTML = '';
  const closeBtn = document.createElement('button');
  closeBtn.className = 'doc-tooltip-close';
  closeBtn.textContent = '✕';
  closeBtn.addEventListener('click', hideDocTooltip);
  docTooltipEl.appendChild(closeBtn);

  senses.forEach(sense => {
    const div = document.createElement('div');
    div.className = 'doc-sense';
    div.innerHTML = `
      <div class="doc-kind">${sense.kind}</div>
      <div class="doc-sig">${sense.sig}</div>
      <div class="doc-desc"></div>
      ${sense.example ? '<div class="doc-example"></div>' : ''}
    `;
    div.querySelector('.doc-desc').textContent = sense.desc;
    if (sense.example) div.querySelector('.doc-example').textContent = sense.example;
    docTooltipEl.appendChild(div);
  });

  // posicionar cerca del punto tocado, sin salirse de la pantalla
  docTooltipEl.classList.add('open');
  const codeAreaRect = codeAreaEl.getBoundingClientRect();
  let left = clientX - codeAreaRect.left + 12;
  let top = clientY - codeAreaRect.top + 16;
  const maxLeft = codeAreaEl.clientWidth - docTooltipEl.offsetWidth - 8;
  if (left > maxLeft) left = Math.max(8, maxLeft);
  const maxTop = codeAreaEl.clientHeight - docTooltipEl.offsetHeight - 8;
  if (top > maxTop) top = Math.max(8, clientY - codeAreaRect.top - docTooltipEl.offsetHeight - 10);
  docTooltipEl.style.left = left + 'px';
  docTooltipEl.style.top = top + 'px';
}

// El modo consulta evita pelear con los gestos nativos del celular
// (mantener presionado abre el menú de Cortar/Copiar/Pegar del sistema, y
// el doble-tap es incómodo de acertar). Con el botón 🔍 activado, el
// editor se pone de solo lectura (así ni siquiera aparece el teclado) y
// un solo toque sobre una palabra muestra su documentación.
...
function enterInspectMode() {
  ...
  codeAreaEl.classList.add('inspect-mode');
...
function exitInspectMode() {
  ...
  codeAreaEl.classList.remove('inspect-mode');
  inspectBtn.classList.remove('active');
  hideDocTooltip();
}

inspectBtn.addEventListener('click', () => {
  if (inspectMode) exitInspectMode(); else enterInspectMode();
});

codeEl.addEventListener('click', () => {
  if (!inspectMode) return;
  const word = getWordAtPosition(codeEl.selectionStart);
  if (!word || !DOCS[word]) { hideDocTooltip(); return; }
  const coords = getCaretCoords(codeEl, codeEl.selectionStart);
  const codeElRect = codeEl.getBoundingClientRect();
  showDocTooltip(word, codeElRect.left + coords.left, codeElRect.top + coords.top);
});

document.addEventListener('click', (e) => {
  if (!docTooltipEl.contains(e.target) && e.target !== codeEl) hideDocTooltip();
});
document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { hideDocTooltip(); exitInspectMode(); } });
...
// Detección básica (no perfecta, no puede serlo) de bucles casi seguros de
// quedarse pegados: un "while true:" / "while 1:" que no tiene ningún
// "break" en todo su bloque. No agarra TODOS los bucles infinitos posibles
// (ej. "while x > 0:" si x nunca cambia), pero sí el error más común.
function checkInfiniteLoopRisk(code) {
  const rawLines = code.split('\n');
  const warnings = [];
  for (let i = 0; i < rawLines.length; i++) {
    const noComment = stripComment(rawLines[i].replace(/\t/g, '    '));
    ...
    for (let j = i + 1; j < rawLines.length; j++) {
      const jNoComment = stripComment(rawLines[j].replace(/\t/g, '    '));
      const jTrimmed = jNoComment.trim();
      if (jTrimmed === '') continue;
      const jIndent = jNoComment.length - jNoComment.trimStart().length;
      if (jIndent <= indent) break; // salimos del bloque de este while
      if (/\bbreak\b/.test(jTrimmed)) { hasBreak = true; break; }
    }
    if (!hasBreak) {
      warnings.push(`Línea ${i + 1}: "while true:" sin ningún "break" adentro — así nunca va a parar.`);
    }
  }
  return warnings;
}
...
function printLoopRiskBanner(warnings) {
  const div = document.createElement('div');
  div.className = 'loop-risk-banner';
  div.textContent = '⚠ Riesgo de bucle infinito — no se ejecutó:\n' + warnings.map(w => '· ' + w).join('\n') + '\n\nAgrega un "break" para salir del bucle en algún momento, o cambia "while true:" por una condición que se vuelva falsa.';
  consoleEl.appendChild(div);
}
...
function executeCode() {
  ...
  const loopWarnings = checkInfiniteLoopRisk(code);
  if (loopWarnings.length > 0) {
    printLoopRiskBanner(loopWarnings);
    return;
  }
...
function enterDebugMode() {
  exitSceneMode();
  exitInspectMode();
  if (typeof closeExercisePanel === 'function' && exercisePanelEl.classList.contains('visible')) closeExercisePanel();
  const code = codeEl.value;
  const { errors, warnings } = checkIndentation(code);
  clearConsole();
  if (errors.length > 0) {
...
// guardado inmediato por si se cierra la página antes de que el debounce dispare
window.addEventListener('pagehide', saveCodeNow);
window.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') saveCodeNow(); });

// ---- escena jugable (Character2D) ----
let sceneActive = false;
let sceneRuntime = null;

function resizeSceneCanvas() {
  // tamaño de dibujo interno fijo (600x400, lo que usa scene.js para las
  // coordenadas); acá solo calculamos el tamaño VISUAL en pantalla, a mano,
  // porque el aspect-ratio de CSS no se comporta igual en todos los
  // navegadores móviles. Además lo topamos por alto disponible, para que
  // los controles táctiles (que flotan sobre el canvas) siempre queden a
  // la vista sin tener que hacer scroll.
  const wrapWidth = sceneViewEl.clientWidth - 24;
  const availableHeight = Math.max(160, window.innerHeight - sceneViewEl.getBoundingClientRect().top - 90);
  let w = Math.min(600, wrapWidth);
  let h = w * (400 / 600);
  ...
  h = Math.round(w * (400 / 600));
  sceneCanvasEl.style.width = w + 'px';
  sceneCanvasEl.style.height = h + 'px';
}
window.addEventListener('resize', () => { if (sceneActive) resizeSceneCanvas(); });

function enterSceneMode() {
  exitDebugMode();
  exitInspectMode();
  if (typeof closeExercisePanel === 'function' && exercisePanelEl.classList.contains('visible')) closeExercisePanel();
  const template = SCENE_TEMPLATES.character2d;

  // si el código ya parece de esta escena, lo dejamos tal cual (para no
  // pisar lo que el usuario ya venía escribiendo); si no, cargamos la base.
  if (!codeEl.value.includes('_physics_process')) {
    setEditorValue(template.starterCode, 0, true);
  }
  ...
  consoleEl.style.display = 'none';
  sceneViewEl.style.display = 'flex';
  sceneTouchControlsEl.classList.add('visible');
  consolePaneLabelEl.textContent = 'escena';
  sceneErrorEl.style.display = 'none';
  scenePlayBtn.disabled = false;
  sceneStopBtn.disabled = true;
  resizeSceneCanvas();
  const ctx = sceneCanvasEl.getContext('2d');
  ctx.clearRect(0, 0, sceneCanvasEl.width, sceneCanvasEl.height);
}

function exitSceneMode() {
  if (!sceneActive) return;
  if (sceneRuntime) { sceneRuntime.stop(); sceneRuntime = null; }
  sceneActive = false;
  runBtn.disabled = false;
  codeEl.disabled = false;
  sceneBtn.classList.remove('active');
  consoleEl.style.display = '';
  sceneViewEl.style.display = 'none';
  sceneTouchControlsEl.classList.remove('visible');
  consolePaneLabelEl.textContent = 'salida';
}

function showSceneError(msg) {
  sceneErrorEl.style.display = 'block';
  sceneErrorEl.textContent = '✗ ' + msg;
}

function playScene() {
  const { errors } = checkIndentation(codeEl.value);
  if (errors.length > 0) {
    showSceneError(errors[0]);
    return;
  }
  const loopWarnings = checkInfiniteLoopRisk(codeEl.value);
  if (loopWarnings.length > 0) {
    showSceneError(loopWarnings[0] + ' Agrega un "break", o la escena se va a colgar.');
    return;
  }
  sceneErrorEl.style.display = 'none';
  scenePlayBtn.disabled = true;
  sceneStopBtn.disabled = false;
  codeEl.disabled = true;
  sceneRuntime = runScene('character2d', codeEl.value, sceneCanvasEl, {
    onPrint: () => { /* en la escena no mostramos consola, para no saturar a 60fps */ },
    onError: (msg) => {
      showSceneError(msg);
      scenePlayBtn.disabled = false;
      sceneStopBtn.disabled = true;
...
sceneBtn.addEventListener('click', () => {
  if (sceneActive) exitSceneMode(); else enterSceneMode();
});
sceneExitBtn.addEventListener('click', exitSceneMode);
scenePlayBtn.addEventListener('click', playScene);
sceneStopBtn.addEventListener('click', stopScene);

// ---- botones táctiles (mover / saltar) para cuando no hay teclado ----
function wireTouchButton(btnEl, action) {
  const press = (e) => {
    e.preventDefault();
    btnEl.classList.add('pressed');
    if (sceneRuntime && sceneRuntime.input) sceneRuntime.input.press(action);
  };
  const release = (e) => {
    e.preventDefault();
    btnEl.classList.remove('pressed');
    if (sceneRuntime && sceneRuntime.input) sceneRuntime.input.release(action);
  };
...
wireTouchButton(sceneTouchLeftBtn, 'ui_left');
wireTouchButton(sceneTouchRightBtn, 'ui_right');
wireTouchButton(sceneTouchJumpBtn, 'ui_select');
