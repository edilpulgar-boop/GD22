...
// ---- documentación al tocar/doble-click una palabra ----
...
// Convierte un punto de la pantalla (clientX/clientY) a un índice de
// caracter dentro del texto, usando el highlight-layer (que es texto real
// del DOM, a diferencia del textarea) como referencia de medida.
function getTextIndexFromPoint(clientX, clientY) {
  let pointRange = null;
  if (document.caretRangeFromPoint) {
    pointRange = document.caretRangeFromPoint(clientX, clientY);
  } else if (document.caretPositionFromPoint) {
    const pos = document.caretPositionFromPoint(clientX, clientY);
    if (pos) { pointRange = document.createRange(); pointRange.setStart(pos.offsetNode, pos.offset); }
  }
  if (!pointRange || !highlightCodeEl.contains(pointRange.startContainer)) return null;
  const measureRange = document.createRange();
  measureRange.setStart(highlightCodeEl, 0);
...
function tryShowDocsAtPoint(clientX, clientY) {
  const idx = getTextIndexFromPoint(clientX, clientY);
  if (idx === null) return;
  const word = getWordAtPosition(idx);
  if (!word || !DOCS[word]) return;
  showDocTooltip(word, clientX, clientY);
}

// Desktop / mouse: doble click sobre una palabra
...
// Táctil: mantener presionado sobre una palabra
let docLongPressTimer = null;
let docLongPressStart = null;
codeEl.addEventListener('touchstart', (e) => {
  if (e.touches.length !== 1) return;
  const touch = e.touches[0];
  docLongPressStart = { x: touch.clientX, y: touch.clientY };
  docLongPressTimer = setTimeout(() => {
    docLongPressTimer = null;
    tryShowDocsAtPoint(touch.clientX, touch.clientY);
  }, 480);
});
codeEl.addEventListener('touchmove', (e) => {
  if (!docLongPressTimer || !docLongPressStart) return;
  const touch = e.touches[0];
  const dx = touch.clientX - docLongPressStart.x, dy = touch.clientY - docLongPressStart.y;
  if (Math.hypot(dx, dy) > 10) { clearTimeout(docLongPressTimer); docLongPressTimer = null; }
});
codeEl.addEventListener('touchend', () => {
  if (docLongPressTimer) { clearTimeout(docLongPressTimer); docLongPressTimer = null; }
});

document.addEventListener('click', (e) => {
  if (!docTooltipEl.contains(e.target) && e.target !== codeEl) hideDocTooltip();
});
codeEl.addEventListener('input', hideDocTooltip);
document.addEventListener('keydown', (e) => { if (e.key === 'Escape') hideDocTooltip(); });
...
document.getElementById('clearCodeBtn').addEventListener('click', () => {
  ...
  if (confirm('¿Borrar todo el código del editor? (podrás deshacerlo con Ctrl+Z / ↶ Deshacer)')) {
