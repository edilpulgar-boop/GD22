...
// Cuenta cuántos paréntesis/corchetes/llaves quedan abiertos (o cerrados de
// más, si da negativo) en un fragmento de texto, ignorando lo que esté
// dentro de strings. Sirve para saber si una línea "sigue" en la próxima
// (ej. un diccionario o array escrito en varias líneas).
...
function splitLines(code) {
  ...
  while (i < rawLines.length) {
    ...
    // Si quedan paréntesis/corchetes/llaves sin cerrar, es una continuación:
    // seguir juntando las próximas líneas hasta que balanceen. Esto permite
    // escribir diccionarios, arrays o llamadas largas en varias líneas.
...
function tokenize(expr) {
  ...
  while (i < n) {
    ...
    if (/[A-Za-z_]/.test(c)) {
      ...
      const kw = ['and', 'or', 'not', 'in', 'true', 'false', 'null'];
  ...
  function parseComparison() {
    ...
    while (true) {
      if (peek().type === 'op' && ['==', '!=', '<', '>', '<=', '>='].includes(peek().value)) {
        ...
        next();
        left = { type: 'in', left, right: parseAdd() };
      } else if (peek().type === 'not' && tokens[pos + 1] && tokens[pos + 1].type === 'in') {
        next(); next();
        left = { type: 'not', value: { type: 'in', left, right: parseAdd() } };
      } else {
        break;
      }
    }
...
function gdStr(v) {
  ...
  if (typeof v === 'object' && v.__gd_input) return `<Input>`;
  if (typeof v === 'object') {
...
class Interpreter {
  ...
  eval(node, scope) {
    ...
    switch (node.type) {
      ...
      case 'in': {
        ...
        throw new Error(`'in' no se puede usar con ese tipo de valor`);
      }
      case 'cmp': {
  ...
  callMethod(obj, name, args) {
    ...
    if (obj && typeof obj === 'object' && obj.__gd_input) {
      if (name === 'is_action_pressed') return obj.heldActions.has(args[0]);
      if (name === 'is_action_just_pressed') return obj.justPressedActions.has(args[0]);
      throw new Error(`Input no tiene el método '.${name}()'`);
    }
